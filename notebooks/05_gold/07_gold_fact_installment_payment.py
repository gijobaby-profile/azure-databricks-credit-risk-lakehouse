...
# MAGIC %md
# MAGIC # Build fact_installment_payment
...
entity_name = "fact_installment_payment"
target_table_full_name = f"{catalog_name}.gold.fact_installment_payment"
...
log_file_path = (
    ...
    f"pipeline/gold/fact_installment_payment/{business_dt}/{pipeline_run_id}.log"
)
logger = get_logger(name=f"gold_fact_installment_payment", log_file_path=log_file_path)

config_for_error = {
    "entity_name": entity_name,
    "source_system": "home_credit",
    "source_path": "silver.conformed_installment_payment",
...
SELECT
    dc.customer_dim_key,
    ip.customer_id,
    ip.previous_application_id,
    ip.installment_version,
    ip.installment_number,
    ip.installment_amount,
    ip.payment_amount,
    ip.payment_delay_days,
    ip.payment_amount_diff,
    ip.is_late_payment,
    ip.is_underpayment,
    ip.business_dt,
...
FROM {catalog_name}.silver.conformed_installment_payment ip

LEFT JOIN {catalog_name}.gold.dim_customer dc
    ON ip.customer_id = dc.customer_id
   AND ip.business_dt = dc.business_dt
   
WHERE ip.business_dt = DATE('{escape_sql(business_dt)}')
