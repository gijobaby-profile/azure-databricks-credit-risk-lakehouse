...
# MAGIC %md
# MAGIC # Build fact_credit_application
...
entity_name = "fact_credit_application"
target_table_full_name = f"{catalog_name}.gold.fact_credit_application"
...
log_file_path = (
    ...
    f"pipeline/gold/fact_credit_application/{business_dt}/{pipeline_run_id}.log"
)
logger = get_logger(name=f"gold_fact_credit_application", log_file_path=log_file_path)

config_for_error = {
    "entity_name": entity_name,
    "source_system": "home_credit",
    "source_path": "silver.conformed_loan_application + gold.dim_*",
...
(
	...
	, high_credit_income_flag 
	, high_annuity_income_flag 
	, business_dt 
	, pipeline_run_id 
...
SELECT
    dc.customer_dim_key,
    dla.loan_application_dim_key,
    la.customer_id,
    la.application_source,
    CAST(1 AS BIGINT) AS application_count,
    la.target AS default_flag,
    la.credit_amount,
    la.annuity_amount,
    la.goods_price,
    la.income_total,
    la.credit_income_ratio,
    la.annuity_income_ratio,
    CASE WHEN la.credit_income_ratio >= 5 THEN true ELSE false END AS high_credit_income_flag,
    CASE WHEN la.annuity_income_ratio >= 0.4 THEN true ELSE false END AS high_annuity_income_flag,
    la.business_dt,
    '{escape_sql(pipeline_run_id)}' AS pipeline_run_id,
    current_timestamp(),
    current_date()
	
FROM {catalog_name}.silver.conformed_loan_application la

LEFT JOIN {catalog_name}.gold.dim_customer dc
    ON la.customer_id = dc.customer_id
   AND la.application_source = dc.application_source
   AND la.business_dt = dc.business_dt
   
LEFT JOIN {catalog_name}.gold.dim_loan_application dla
    ON la.customer_id = dla.customer_id
   AND la.application_source = dla.application_source
   AND la.business_dt = dla.business_dt
   
WHERE la.business_dt = DATE('{escape_sql(business_dt)}')
